...
CHRONIC_ALL_GDAY =          pathlib.Path(RAWDATA_DIR, "Acute Food Consumption Grams in a single day – All days.xlsx")
CHRONIC_CONSUMERS_GDAY =    pathlib.Path(RAWDATA_DIR, "Acute Food Consumption Grams in a single day – Consuming days only.xlsx")
CHRONIC_ALL_GKGDAY =        pathlib.Path(RAWDATA_DIR, "Acute Food Consumption Grams per kilogram of body weight gkg bw in a single day – All days.xlsx")
CHRONIC_CONSUMERS_GKGDAY =  pathlib.Path(RAWDATA_DIR, "Acute Food Consumption Grams per kilogram of body weight gkg bw in a single day – Consuming days only.xlsx")
...
class ConsumptionData:
    def __init__(self, abrv, filepath, IDENTIFIER_COLUMNS, DATA_COLUMNS):
        ...
        # extra rename
        self.df = self.df.rename(columns={"survey__name": "survey_name"})
...
COLUMN GROUPINGS
"""
IDENTIFIER_COLUMNS = ['surveys_country', 'survey_start_year', 'survey_name', 'population_group_l2', 'exposure_hierarchy_l7', 'gender']
...
"""
cag = ConsumptionData('aag', CHRONIC_ALL_GDAY, IDENTIFIER_COLUMNS, DATA_COLUMNS)
ccg = ConsumptionData('acg', CHRONIC_CONSUMERS_GDAY, IDENTIFIER_COLUMNS, DATA_COLUMNS)
cagkg = ConsumptionData('aagkg', CHRONIC_ALL_GKGDAY, IDENTIFIER_COLUMNS, DATA_COLUMNS)
ccgkg = ConsumptionData('acgkg', CHRONIC_CONSUMERS_GKGDAY, IDENTIFIER_COLUMNS, DATA_COLUMNS)
...
"""
copy_query = f"\copy efsa_consumption.acute_gender({','.join(df.columns)}) from '{FINAL_SAVEPATH}' (header true, delimiter ',', format csv, encoding 'UTF-8');"
print(copy_query)
